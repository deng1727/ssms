...
alter table v_cm_device_resource add THIRDCOLLECT VARCHAR2(1) default '0';
alter table v_cm_device_resource_mid add THIRDCOLLECT VARCHAR2(1) default '0';

alter table t_a_cm_device_resource add THIRDCOLLECT VARCHAR2(1) default '0';
  ...
  v_nstatus := pg_log_manage.f_startlog('f_ppms_cm_device_pid',
    ...
    if v_nrecod > 0 then
      ...
      insert into v_cm_device_resource
        select pid,device_id,device_name,contentid,contentname,resourceid,id,servicecode,absolutepath,url,cdnurl,wapurl,mourl,programsize,jarfilepath,jadfilepath,contentuuid,createdate,prosubmitdate,match,version,permission,iscdn,versionname,caflag,cadev,caorg,cavalidatedate,pcurl,wwwurl,thirdcollect

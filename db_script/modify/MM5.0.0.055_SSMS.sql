...
delete t_v_dprogram d where rowid < (select max(rowid) from  t_v_dprogram dp where dp.programid = d.programid and dp.cmsid = d.cmsid)

alter table T_V_DPROGRAM
  add constraint PK_TVDPROGRAM_PROGRAM unique (PROGRAMID, CMSID);
...
insert into dbversion(DBSEQ,DBVERSION,LASTDBVERSION,PATCHVERSION) values (SEQ_DB_VERSION.nextval,'1.0.0.0','MM5.0.0.0.309_SSMS','MM5.0.0.0.505_SSMS');
commit;
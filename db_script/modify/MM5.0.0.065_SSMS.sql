...
where gc.contentid = b.contentid and gc.appcateid = v.appcateid and
 b.contentid = m1.appid and v.sortid = b.sortid and b.type = '5');
 update mid_table m
   set m.hotlist = (-0.000001)* (select sortid
                        from (select m.appid,
                                     row_number() over(order by g.plupddate desc) sortid
                                from mid_table m, t_r_gcontent g
                               where m.hotlist = 0
                                 and g.contentid = m.appid) mg
                       where mg.appid = m.appid)
 where m.hotlist = 0;
 commit;
...
insert into dbversion(DBSEQ,DBVERSION,LASTDBVERSION,PATCHVERSION) values (SEQ_DB_VERSION.nextval,'1.0.0.0','MM5.0.0.0.059_SSMS','MM5.0.0.0.065_SSMS');
commit;
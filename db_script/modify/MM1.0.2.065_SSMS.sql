...
select p.contentid,
 ...
 where p.contentid = c.contentid
   ...
   and p.id = v2.id
   and (c.thirdapptype in ('1', '2','6','7') or (c.thirdapptype = '5' and c.Jilstatus = '1'))
  UNION ALL
 select
       t.contentid,
       t.icpcode,
...
--创建重点货架
create table T_R_EMPHA_CATEGORY
(
...
comment on column T_R_EMPHA_CATEGORY.CATEGORYID
  is '货架id';
comment on column T_R_EMPHA_CATEGORY.CATEGORYNAME
  is '货架名称';
comment on column T_R_EMPHA_CATEGORY.TIME
  is '时间';
comment on column T_R_EMPHA_CATEGORY.FLAG
  is '标识';

insert into dbversion(DBSEQ,DBVERSION,LASTDBVERSION,PATCHVERSION) values (SEQ_DB_VERSION.nextval,'1.0.0.0','MM1.0.2.060_SSMS','MM1.0.2.065_SSMS');
commit;
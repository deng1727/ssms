alter table T_R_REFERENCE drop column VARIATION;
--还原以前节点类型
update t_r_category c set c.ctype=1 where c.name='终端音乐推荐' and c.parentcategoryid is null;
update t_r_category c set c.ctype=0 where c.parentcategoryid= (select c.categoryid from t_r_category c where c.name='终端音乐推荐' and c.parentcategoryid is null);
commit;
alter table T_R_CATEGORY drop column picurl ;
alter table T_R_CATEGORY drop column statistic ;

delete from t_resource t where t.resourcekey='RESOURCE_CATE_BO_UPLOAD_1010';

alter table T_GAME_SERVICE drop column contentid ;
...
delete DBVERSION where PATCHVERSION = 'MM1.0.0.080_SSMS' and LASTDBVERSION = 'MM1.0.0.077_SSMS';
commit;
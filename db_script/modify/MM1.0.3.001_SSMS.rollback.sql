...
comment on column T_CATERULE_COND.CONDTYPE
  is '条件类型 10：从产品库获取自有业务（软件，游戏，主题）；12：从产品库获取非自有业务；1：精品库获取。';

delete from t_category_rule t where t.ruleid=201;
delete from t_caterule_cond t where t.ruleid=201;
delete from t_caterule t where t.ruleid=201;


delete DBVERSION where PATCHVERSION = 'MM1.0.3.001_SSMS' and LASTDBVERSION = 'MM1.0.2.100_SSMS';
commit;
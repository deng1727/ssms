...
if(id==null){
	...
	List nodeList = TreeNodeManager.getchildrenNodesWpInfo(null);
	out.println("[");
	for(int i=0;i<nodeList.size();i++){
		TreeNode node = (TreeNode)nodeList.get(i);
		out.print("{id:"+node.getId()+",pId:"+node.getParentId()+",name:'"+node.getName()+"',url:'../wpInfo/appInfoCategory.do?perType=query&categoryId="+node.getId()+"&cgyPath="+TreeNodeManager.getPath(node.getId())+"',isParent:'true',target:'mainFrame1'}");	
...
List nodeList = TreeNodeManager.getchildrenNodesWpInfo(id);
if(nodeList.size()>0){
	out.println("[");
	for(int i=0;i<nodeList.size();i++){
		TreeNode node = (TreeNode)nodeList.get(i);

...
<%@page import="com.aspire.ponaadmin.web.repository.CategoryApprovalVO"%>
<%@page
...
function passApproval() {
	...
	if (window.confirm("确定审核通过？")) {
		window.location.href = "categoryApproval.do?operation="+operation+"&status=1&categoryId="
				+ categoryId;
	} else {
		return;
	}
...
function noPassApproval() {
	...
	if (window.confirm("确定审核不通过？")) {
		window.location.href = "categoryApproval.do?operation="+operation+"&status=3&categoryId="
				+ categoryId;
	} else {
		return;
	}
...
function showCategoryInfo(categoryID) {
	window
			.open("categoryInfo.do?approvalFlag=yes&Popup=Popup&categoryID=" + categoryID,
					"new",
					"width=600,height=600,toolbar=no,scrollbars=yes,menubar=no,resizable=true ")
}

function detail(id){
	 var isReflesh= window.showModalDialog("categoryGoodsList.do?subSystem=ssms&categoryID=" + id,"modal","center:yes;dialogWidth:800px;dialogHeight:800px;resizable: yes; help: no; status: yes; scroll: auto; ");
	  if(isReflesh)
	  {
	...
	<body bgcolor="#F0FBFD" leftmargin="0" topmargin="0">
		...
		<table width="95%" border="0" align="center" cellspacing="1">
			<form name="queryUserForm"
				action="categoryApprovalList.do?operation=<%=operation%>"
				...
				<tr align="center" class="<%=(i % 2 == 0) ? "text5" : "text1"%>">
					<td>
						...
						<%if(!"2".equals(categoryApprovalVO.getApprovalStatus())) {%>disabled="disabled"/<%} %>
							value="<%=categoryApprovalVO.getId()%>" />
					</td>
					<td>
						<a href="#"
							<logic:equal value="1" name="operation"> onclick="showCategoryInfo('<%=categoryApprovalVO.getId()%>')" </logic:equal>
							<logic:equal value="2" name="operation"> onclick="detail('<%=categoryApprovalVO.getId()%>')" </logic:equal>><%=categoryApprovalVO.getCategoryId()%></a>
					</td>
					<td>
						<a href="#"
							<logic:equal value="1" name="operation"> onclick="showCategoryInfo('<%=categoryApprovalVO.getId()%>')" </logic:equal>
							<logic:equal value="2" name="operation"> onclick="detail('<%=categoryApprovalVO.getId()%>')" </logic:equal>><%=categoryApprovalVO.getCategoryName()%></a>
					</td>
			...
			<table width="95%" border="0" align="center" cellspacing="3"
				...
				%>
						<pager:pager name="PageResult"
							action="/web/resourcemgr/categoryApprovalList.do" params="<%=params%>">

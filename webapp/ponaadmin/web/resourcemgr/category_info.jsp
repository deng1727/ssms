...
{
  window.location.href = "categoryEdit.do?action=new&approvalFlag=<%=approvalFlag%>&pCategoryID=<bean:write name="category" property="id"/>&cgyPath=<%=cgyPath%>"+
                         "&ctype=<bean:write name="category" property="ctype"/>";
}

function modCategory()
{
  window.location.href = "categoryEdit.do?action=update&approvalFlag=<%=approvalFlag%>&pCategoryID=<bean:write name="category" property="parentID"/>&categoryID=<bean:write name="category" property="id"/>&cgyPath=<%=cgyPath%>"+
                         "&ctype=<bean:write name="category" property="ctype"/>";
}
...
function delCategory()
{
  ...
  {
    window.location.href = "categoryDel.do?approvalFlag=<%=approvalFlag%>&pCategoryID=<bean:write name="category" property="parentID"/>&categoryID=<bean:write name="category" property="id"/>&cgyPath=<%=cgyPath%>";
    
  }
}
 
function submitApproval(){
	window.location.href = "categoryApproval.do?operation=1&status=2&categoryId=<bean:write name="category" property="id"/>&cgyPath=<%=cgyPath%>";
}
...
<table width="95%"  border="0" align="center" cellspacing="1" bgcolor="#FFFFFF">
 ...
 <logic:iterate id="vo" indexId="ind" name="keyResourceList"  type="com.aspire.ponaadmin.web.datafield.vo.ResourceVO">
	...
	<logic:equal value="yes" name="approvalFlag">
	<tr>
    <td align="right" class="text3">审批状态： </td>
    <td class="text4">
    	<logic:equal value="0" name="category" property="classifyStatus"> 编辑 </logic:equal>
    	<logic:equal value="1" name="category" property="classifyStatus"> 已发布 </logic:equal>
    	<logic:equal value="2" name="category" property="classifyStatus"> 待审批 </logic:equal>
    	<logic:equal value="3" name="category" property="classifyStatus"> 不通过 </logic:equal>
    </td>
  </tr>
  </logic:equal>
</table>
<!-- 如果不是弹出窗口，才需要显示一下按钮  -->
<logic:notEqual value="Popup" name="Popup" >
<table width="95%"  border="0" align="center" cellspacing="1" bgcolor="#FFFFFF">
  <tr>
    <td align="center" class="text5">
        <input name="button1" type="button" class="input1" value="新增" onclick="return addCategory();">
        <input name="Submit" type="submit" class="input1" <logic:equal value="2" name="category" property="classifyStatus">style="color: #969696;" disabled="disabled"</logic:equal> value="修改属性" onclick="return modCategory();">
        <logic:notEqual name="category" property="ctype" value="1"><input name="Submit" type="submit" class="input1" <logic:equal value="2" name="category" property="classifyStatus">style="color: #969696;" disabled="disabled"</logic:equal> value="删除" onclick="return delCategory();"></logic:notEqual>
        <logic:equal value="yes" name="approvalFlag">
        	<input name="button1" type="button" class="input1" <logic:notEqual value="0" name="category" property="classifyStatus">style="color: #969696;" disabled="disabled"</logic:notEqual> value="提交审批"  onclick="return submitApproval();">
        </logic:equal>
    </td>
  </tr>
</table>

...
if(id==null){
	...
	List nodeList = TreeNodeManager.getchildrenNodesVideoNew(null,"1");
	out.println("[");
	for(int i=0;i<nodeList.size();i++){
		TreeNode node = (TreeNode)nodeList.get(i);
		out.print("{id:"+node.getId()+",pId:"+node.getParentId()+",name:'"+node.getName()+"',url:'../newVideo/videoReference.do?perType=query&categoryId="+node.getId()+"',isParent:'true',target:'mainFrame1'}");	
...
List nodeList = TreeNodeManager.getchildrenNodesVideoNew(id,"1");
if(nodeList.size()>0){
	out.println("[");
	for(int i=0;i<nodeList.size();i++){
		TreeNode node = (TreeNode)nodeList.get(i);

...
function addBlack(){
    self.location="<%=request.getContextPath()%>/web/blacklist/black_add.jsp";
}
function tactic()
{
...
function removeContent(contentID,blacklistStatus)
{

  if(confirm("确定从黑名单中移除该内容？"))
  {
    contentForm.action="blackMain.do?action=del&contentId="+contentID+"&blacklistStatus="+blacklistStatus;
    contentForm.submit();
  }
}
function toUpdate(contentId){
    contentForm.action="blackMain.do?action=toUpdate&contentId="+contentId;
    contentForm.submit();
}
...
function submitApproval(){
	 ...
	 contentForm.action="<%=request.getContextPath()%>/web/blacklist/approval.do?action=submitApproval";
     contentForm.submit();
 }
...
<table width="95%" border="0" align="center" cellspacing="1">
	...
	<form name="contentListForm" action="black.do" method="post"
...
<logic:notEmpty name="PageResult" property="pageInfo">
	<table width="95%" border="0" align="center" cellspacing="1"
        ...
        %> <pager:pager name="PageResult"
				action="/web/blacklist/black.do" params="<%=params%>">
	...
	<table width="95%" border="0" align="center" cellspacing="1"
		...
		<tr>
            ...
            <td width="10%" align="center" class="title1">开始时间</td>
			<td width="10%" align="center" class="title1">截止时间</td>
			<td width="10%" align="center" class="title1">类型</td>
			<td width="10%" align="center" class="title1">审批状态</td>
			<td width="5%" align="center" class="title1">修改</td>
			<td width="5%" align="center" class="title1">删除</td>
		</tr>

			<%String tmpStyle = "text5";%>
		<logic:iterate id="vo" indexId="ind" name="PageResult"
			property="pageInfo"
			type="com.aspire.ponaadmin.web.blacklist.vo.BlackListVO">
	...
	%>
			<tr class=<%=tmpStyle%>>
                <td align="center"><input type="checkbox" name="dealContent" value="<bean:write name="vo" property="contentId" />" <%if(!("0".equals(vo.getBlacklistStatus()) || "3".equals(vo.getBlacklistStatus()))){ %> disabled="true" <%} %>></td>
	   ...
	   <%} %>
				...
				<td align="center" style="word-break: break-all">
				    <input name="ad" type="button" class="input1" onClick="toUpdate('<bean:write name="vo" property="contentId" />');" <% if("2".equals(vo.getBlacklistStatus())){ %> disabled="disabled" <%} %> value="修改"/>
				</td>
				<td align="center" style="word-break: break-all">
				    <input name="ad" type="button" class="input1" onClick="removeContent('<bean:write name="vo" property="contentId" />','<bean:write name="vo" property="blacklistStatus" />');" <% if("2".equals(vo.getBlacklistStatus())){ %> disabled="disabled" <%} %> value="删除"/>
				</td>				
			</tr>
		</logic:iterate>
	</table>
...
<table width="95%" border="0" align="center" cellspacing="1"
	...
	<tr>
		<td align="center" class="text5">
		    <input name="ad" type="button" class="input1" onClick="addBlack();" value="新增黑名单"/>
		    <input name="ad" type="button" class="input1" onClick="self.location='<%=request.getContextPath()%>/web/blacklist/black_import.jsp';" value="导入黑名单"/>
		    <logic:notEmpty name="PageResult" property="pageInfo"><input name="buttonOperation" type="button" class="input1" onClick="return submitApproval();" value="提交审批"></logic:notEmpty>
		</td>
	</tr>
</table>
</body>

...
function WebFXLoadTree(mykey,myext,sText, sXmlSrc, sAction, sBehavior, sIcon, sOpenIcon) {
	// call super
	this.WebFXTree = WebFXTree;
	this.WebFXTree(mykey,myext,sText, sAction, sBehavior, sIcon, sOpenIcon);
	...
	if (this.open)
		...
		this._loadingItem = new WebFXTreeItem("-1","0","0","...正在加载...");
...
function WebFXLoadTreeItem(mykey,myext,myFolder,sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(mykey,myext,myFolder,sText, sAction, eParent, sIcon, sOpenIcon);
...
WebFXLoadTreeItem.prototype.expand = function() {
	//alert("expand() WebFXLoadTreeItem");
	if (!this.loaded && !this.loading) {
		// load
		_startLoadXmlTree(this.src, this);
	}
...
WebFXLoadTreeItem.prototype.reload = function () {
	...
	if (this.loaded) {
		...
		//@@@@@@@@@@@@@@@@@@@@@@@@@@
		this._loadingItem = new WebFXTreeItem("-1","0","0",webFXTreeConfig.loadingText);
		this.add(this._loadingItem);
...
function _xmlTreeToJsTree(oNode) {
	// retreive attributes
	var mykey= oNode.getAttribute("key");
	var myext= oNode.getAttribute("ext");
	var myFolder= oNode.getAttribute("folder");
	...
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadTreeItem(mykey,myext,myFolder,text, src, action, parent, icon, openIcon);
	else
		jsNode = new WebFXTreeItem(mykey,myext,myFolder,text, action, parent, icon, openIcon);

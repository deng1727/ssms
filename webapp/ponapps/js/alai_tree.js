...
{try{
	...
	{	
		...
		{	
		    var sEx=srcNode.next==null?"":"_top";
			//if(srcNode==nodes[0])sEx="_end";
			if(srcNode==srcNode.parent.last)sEx="_end";
		...
		{				
            ...
            if(showBranch){srcNode.exIcon.src=srcNode==srcNode.parent.last?icons["twig"].src:icons["leaf"].src}
	...
	{		
		...
		srcNode.label.style.background="#EFF3FF";srcNode.label.style.color="black";
		selectedNode=srcNode
	}
	var doFocus=function(srcNode)
	{	
		if(srcNode==selectedNode){srcNode.label.style.background="#EFF3FF";srcNode.label.style.color="black";}
		activeNode=srcNode
	}
	...
	{		
		...
		{	case "first":
			...
			default:
				...
				node.parent=toNode
				if(toNode.hasChild){node.prev=toNode.last;toNode.last=node;toNode.last=node;}
				else{toNode.first=toNode.last=node;}
		}
		...
		with(label)
		{
			...
			ondblclick=function(){if(run(node.onclick))run(node.load);}
		...
		{			
			...
			if(node.prev!=null && node.next!=null)setLine(node.prev,tier-1)
			var n=node.parent;var i=tier-2;
			while(n!=root)
            {
				if(n.parent.last!=n)node.lineIcon[i].src=icons["line"].src;
				n=n.parent;i--
			}
		}
		...
		node.expand=function(isShow,incSub)
		{
			...
			//判断是不是文件夹
			if(typeof(node.label.sign)=='undefined')return;
		...
		node.load=function(isShow,incSub)
		{
			//判断是不是文件夹
			if(typeof(node.label.sign)=='undefined')return;
			//调用加载
			loadTreeNode(node.label.src,tree,node);
            if(isShow==null)node.expanded=!node.expanded;else node.expanded=isShow;
			incSub=incSub==null?false:incSub
			var sh=node.expanded?"block":"none"
			for(var i=0;i<node.children.length;i++){node.children[i].container.style.display=sh;if(incSub)node.children[i].expand(node.expanded,true);}
		...
		with(node.exIcon)
		{
			onmousedown=function(){if(run(node.onclick))run(node.load);}
		}
        ...
        node.reload=function(mykey)
		{
            if(node.expanded==false)
			{
                ...
                node.children.length=node.children.length-t;
				node.label.sign=0;
				return;
			}
			//alert("该节点有"+node.children.length+"个子节点");
			//for(var i=0;i<node.children.length;i++)
			//{
			//alert("删除第"+(i+1)+"个");
			//node.children[i].remove();
			//alert("Tree的节点数为"+tree.count()+"个");
			//}
            //alert("该节点还剩下"+node.children.length+"个子节点");
		    //node.label.sign=0;
            //loadTreeNode(node.label.src,tree,node);
            var t=node.children.length;
			for(var i=0;i<t;i++)
		...
		{	
			...
			//	}else {node.execute=node.expand;exeCategory="expand";}
 			case "expand":	
				node.execute=node.expand;
			    break; 
			default:
                node.execute=node.expand;exeCategory="expand";
		}
	...
	this.removeNode=function(srcNode){if(srcNode.children.length==0){remove(srcNode);}else{for(var i=0;i<srcNode.children.length;i++){count--;}remove(srcNode);}}
	...
	//展开一节点的子节点
	this.expandTier=function(node){for(var i=0;i<node.children.length;i++)node.children[i].expand(false);}
	...
	this.pathParse=function(key,name,pathList,sort,ps,ls,ico)
	{	
		if(ps==null)ps="/";if(ls==null)ls=";";
		var list=pathList.split(ls);
		...
		//根节点加载
		n=root.add("<A href='javascript:treeControl()'>"+name+"</A>",ico);
        n.label.id=key;
		n.label.innerHTML=n.label.innerHTML+" "+WWWExtend(n);
		n.label.sign=1;
		for(var i=0;i<list.length;i++)
		{	
		  AllList=list[i].split(',');
		  if(AllList[0]!='')
		  {
			 list[i]=AllList[0];
		  }
		  ...
		  if(list[i]=="")continue;var path=list[i].split(ps),node=tree.nodes[0];
		  for(var j=0;j<path.length;j++)
		  {	 
			 if(path[j]==""){continue;};var p="";for(var m=0;m<=j;m++){if(path[m]!=""){p+="/"+path[m];}}
			 idx=pathIndex(p);
			 if(idx!=-1){node=nodes[idx];}
			 else 
		     {  
			    if(typeof(AllList[5])=='undefined'){node=node.add(AllList[2],ico);node.hasChild=false;}
			    else{node=node.add(AllList[2],ico);node.hasChild=true;}
			    colPath[colPath.length]=p;
			 }
		  }
			node.label.name=node.label.innerHTML;
		  //加入key
		  if(typeof(AllList[0])!='undefined')
		  {
		     node.label.id=AllList[0];
		  }
		  //加入folder
		  if(typeof(AllList[1])!='undefined')
		  {
		     node.label.folder=AllList[1];
		  }
		  //加入控制
		  if(typeof(AllList[3])!='undefined')
		  {
		     node.label.innerHTML="<A href='"+AllList[3]+"'>"+node.label.innerHTML+"</A>";
		  }
		  //加入回调
		  if(typeof(AllList[4])!='undefined')
		  {
			 node.label.ext=AllList[4];
             node.label.innerHTML=node.label.innerHTML+" "+WWWExtend(node);
		  }
		  //加入sSrc
		  if(typeof(AllList[5])!='undefined')
		  {
			 node.label.src=AllList[5];
			 //是否加载标记
			 node.label.sign=0;
		  }
		}
	}
	this.loadParse=function(n,pathList,sort,ps,ls,ico)
	{
		...
		{	
		  ...
		  if(list[i]=="")continue;var path=list[i].split(ps),node=n;
		  for(var j=0;j<path.length;j++)
		  {	 
			 if(path[j]==""){continue;};var p="";for(var m=0;m<=j;m++){if(path[m]!=""){p+="/"+path[m];}}
			 idx=pathIndex(p);
	...
	this.embed(toObject)
	//根节点更新
	this.reload=function(mykey)
	{
		if(tree.root.children[0].expanded==false)
		{
            refresh();
		}
        var t=tree.root.children[0].children.length;
		//alert("根目录的节点数："+t);
        for(var i=0;i<tree.root.children[0].children.length;i++)
		{tree.root.children[0].children[i].remove();}
		
        tree.root.children[0].children.length=tree.root.children[0].children.length-t;
		tree.root.children[0].label.sign=0;
        loadTreeNode(url,tree,tree.root.children[0]);
	}
}
catch(e){
...
function findByMykey(nodeObj,mykey)
{	
	if(key==mykey) return nodeObj;

	for(var i=0;i<tree1.nodes.length;i++)
	{
		if(tree1.nodes[i].label.id==mykey)
		{
			return tree1.nodes[i];
		}
	}
}
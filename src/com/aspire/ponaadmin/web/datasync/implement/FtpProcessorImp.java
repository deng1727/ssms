...
public class FtpProcessorImp implements FtpProcessor
{
	private static JLogger logger = LoggerFactory.getLogger(FtpProcessorImp.class);
	...
	{
		this.fNameRegex = config.get("ftp.fNameRegex").split(",");
		this.fName=new String[fNameRegex.length];
		this.ftpDir = config.get("ftp.ftpDir");
		this.localDir = config.get("ftp.localDir");
		this.ip = config.get("ftp.ip");
		this.port = Integer.parseInt(config.get("ftp.port"));
		this.user = config.get("ftp.user");
		this.password = config.get("ftp.password");
	}

	public String[] process()throws BOException
	{
		FTPClient ftp = null;
		try
		{
	...
	//下载指定文件名系列
	public boolean process(String[] fileName)throws BOException
	{
		...
		try
		{
			...
			for(int i=0;i<fileName.length;i++){
				...
				if (logger.isDebugEnabled())
				{
					logger.debug("匹配文件名开始：" + fileName[i]);
				}
				String absolutePath = localDir + File.separator + fileName[i];
				absolutePath = absolutePath.replace('\\', '/');
				ftp.get(absolutePath,fileName[i]);
				logger.debug("成功下载文件：" + absolutePath);
			}
			result = true;
		} catch(Exception e)

...
public class ThirdChannelMkTimer {
	/**
	 * 记录日志的实例对象
	 */
	private static JLogger LOG = LoggerFactory.getLogger(ThirdChannelMkTimer.class);
	private static ThirdChannelMkTimer timer = new ThirdChannelMkTimer();

	private ThirdChannelMkTimer() {

	}

	public static ThirdChannelMkTimer getInstance() {
		return timer;
	}
	...
	public void start() {
		...
		// 从配置项中获取配置值
		try {
			String rbStartTime = ChannelConfig.STARTTIME;
		...
		timer.schedule(new ThirdChannelMkTask(), firstStartTime, taskIntervalMS);
		if (LOG.isDebugEnabled()) {
			LOG.debug("schedule a ThirdChannelMkTask,first start time is:"
					+ firstStartTime);
		}
	}

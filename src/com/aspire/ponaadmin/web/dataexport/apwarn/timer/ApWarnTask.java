...
public class ApWarnTask {
	/**
	 * 记录日志的实例对象
	 */
	private static JLogger LOG = LoggerFactory.getLogger(ApWarnTask.class);
	private static ApWarnTask timer = new ApWarnTask();

	private ApWarnTask() {

	}

	public static ApWarnTask getInstance() {
		return timer;
	}
	...
	public void start() {
		...
		// 从配置项中获取配置值
		try {
			String rbStartTime = ApWarnConfig.STARTTIME;
		...
		Timer apWarnTimer = new Timer(true);
        ...
        apWarnTimer.schedule(new ApWarnTimer(), firstStartTime, taskIntervalMS);
		if (LOG.isDebugEnabled()) {
			LOG.debug("schedule a ApWarnTimer,first start time is:"
					+ firstStartTime);
		}
	}

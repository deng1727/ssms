...
public class BaseFileTimer
{
    /**
     * 记录日志的实例对象
     */
    private static JLogger LOG = LoggerFactory.getLogger(BaseFileTimer.class);
    private static BaseFileTimer timer = new BaseFileTimer();

    private BaseFileTimer()
    {
    }

    public static BaseFileTimer getInstance()
    {
        return timer;
    }
    ...
    public void start()
    {
        ...
        try
        {
            String rbStartTime = BaseFileConfig.STARTTIME;
        ...
        timer.schedule(new BaseFileTask(), firstStartTime, taskIntervalMS);
        if (LOG.isDebugEnabled())
        {
            LOG.debug("schedule a BaseFileTask,first start time is:"
                      + firstStartTime);
        }
    }

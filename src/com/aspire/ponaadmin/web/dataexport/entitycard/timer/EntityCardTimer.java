...
public class EntityCardTimer {
	private static JLogger LOG = LoggerFactory.getLogger(EntityCardTimer.class);
	
	private static EntityCardTimer timer = new EntityCardTimer();
	
	public static EntityCardTimer getInstance(){
		return timer;
	}
	...
	public void start() {
		...
		// 从配置项中获取配置值
		try {
			String rbStartTime = EntityCardConfig.STARTTIME;
			hour = Integer.parseInt(rbStartTime.split(":")[0]);
			minute = Integer.parseInt(rbStartTime.split(":")[1]);
		} catch (Exception ex) {
			LOG.error("从配置项中获取解析 EntitycardStartTime解析出错！");
		}
		if (hour < 0 || hour > 23) {
			// 配置值不正确，就使用默认值
			hour = 5;
		...
		timer.schedule(new EntityCardTask(), firstStartTime, taskIntervalMS);
		if (LOG.isDebugEnabled()) {
			LOG.debug("schedule a EntityCardTask,first start time is:"
					+ firstStartTime);
		}
	}

...
import com.aspire.ponaadmin.web.newmusicsys.vo.CategoryApprovalVO;
...
public class CategoryApprovalListDAO {
	...
	public void queryCategoryApprovalList(PageResult page, Map map)
		...
		String sqlCode = "com.aspire.ponaadmin.web.newmusicsys.dao.CategoryApprovalListDAO.queryCategoryApprovalList";
		...
		try {
			...
			if (!"".equals(approvalStatus) && !"-1".equals(approvalStatus)) {
				/**
				 * 不同的处理。用不用的审批状态标识 1. 音乐货架管理; 2. 音乐商品管理
				 */
				if ("1".equals(operation)) {
					sqlBuffer.append(" and C.music_status = ? ");
	...
	private class CategoryApprovalPageVO implements PageVOInterface {
		...
		public void CopyValFromResultSet(Object content, ResultSet rs)
			...
			if ("1".equals(this.operation)) {
				categoryApproval
						.setApprovalStatus(rs.getString("music_status"));

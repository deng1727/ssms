...
public class CgyContentListDAO
{
	 ...
	 */
	public void getCgyContentList( PageResult page,Category category, Searchor searchor,Taxis taxis,boolean isExistedType,boolean isExistedDevice)throws DAOException
	{
		 String sql;
	    ...
	    //设置查询参数。
	    setSearchSQL(bf, paraList, searchor);
	    //设置排序参数。
	 ...
	 * @param isExistedType boolean 是否搜索条件含有type类型，如果有的话，特殊处理。
	 * @param isIncluded boolean 是否包含本地货架的商品。true表示之查询该货架的商品内容，false表示查询非该货架的商品内容
	 * @param channelsId 渠道Id 
	 */
	public void getCgyNotContentList( PageResult page,Category category, Searchor searchor,Taxis taxis,boolean isExistedType,String channelsId)throws DAOException
	{
		String sql;
		List paraList = new ArrayList();
		if(isExistedType)
		{
			...
			sql =DB.getInstance().getSQLByCode("com.aspire.ponaadmin.web.channelUser.action.CgyContentListDAO.getCgyNotContentList.SELECT");
		}else
		{
			if(LOG.isDebugEnabled())
			{
			...
			sql =DB.getInstance().getSQLByCode("com.aspire.ponaadmin.web.channelUser.action.CgyContentListDAO.getCgyNotContentList.notExistedType.SELECT");
		}
		...
		if(channelsId != null && !"".equals(channelsId)){
			sql=PublicUtil.replace(sql, "{0}", " and g.companyid='" + channelsId + "'");
		}else{
			sql=PublicUtil.replace(sql, "{0}", "");
		}
		StringBuffer bf = new StringBuffer(sql);
		// 设置查询参数。
		setSearchSQL(bf, paraList, searchor);
	...
	private void setReferenceValues(ReferenceNode refNode,ResultSet rs) throws SQLException
	{
		...
		refNode.setVerifyStatus(rs.getString("verify_status"));
	}
	/**
	...
	private void setGContentValues(GContent content,ResultSet rs)throws SQLException
	{
        ...
        try {
        	...
        	content.setVerifyStatus(verifyStatus);
		} catch (Exception e) {
			content.setVerifyStatus("");
		}
	...
	private void setSearchSQL(StringBuffer sql, List paraList,Searchor searchor)
	{
		 // 遍历表达式的各个参数，进行处理

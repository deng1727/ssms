...
public class BaseMusicLoadTimer
{
	
	private static BaseMusicLoadTimer instance = new BaseMusicLoadTimer();
    /**
     * 构造方法
     */
    private BaseMusicLoadTimer ()
    {

    }
public static BaseMusicLoadTimer getInstance(){
	return instance;
}
    /**
     * 记录日志的实例对象
     */
    private static JLogger LOG = LoggerFactory.getLogger(BaseMusicLoadTimer.class) ;
    ...
    public  void start ()
    {
        ...
        try
        {
            String BaseMusiSynStartTime = BaseMusicConfig.get("BaseMusicSynStartTime") ;
            hour = Integer.parseInt(BaseMusiSynStartTime.split(":")[0]);
            minute = Integer.parseInt(BaseMusiSynStartTime.split(":")[1]);
        }
        catch (Exception ex)
        {
            LOG.error("从配置项中获取基地音乐同步的时间BaseMusiSynStartTime解析出错！");
        }
        if(hour<0||hour>23)
        {
            //配置值不正确，就使用默认值
        ...
        timer.schedule(new BaseMusicLoadTask(), firstStartTime, taskIntervalMS) ;
        if (LOG.isDebugEnabled())
        {
            LOG.debug("schedule a BaseMusicLoadTask,first start time is:" + firstStartTime) ;
        }
    }

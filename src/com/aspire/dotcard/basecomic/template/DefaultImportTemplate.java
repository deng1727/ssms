...
public abstract class DefaultImportTemplate {
	
	protected static JLogger logger = LoggerFactory.getLogger(DefaultImportTemplate.class);
				...
				while ((line = br.readLine()) != null) {
			    	...
			    	if(existsAllKey.contains(vo.getKey())){
				    	this.update(queue, vo,new StatisticsCallback(){
							...
							public void doStatistics(boolean isSuccess) {
								...
								if(isSuccess){//JDBC修改操作成功
									...
									mailInfo[5] = mailInfo[5]+1;   //呵呵，这个没有给锁，是因为觉得重要性差，出概率错误也少，故略了。
				    	...
				    	this.insert(queue, vo,new StatisticsCallback(){
							...
							public void doStatistics(boolean isSuccess) {
								// TODO Auto-generated method stub
								if(isSuccess){//JDBC增加操作成功
									existsAllKey.add(vo.getKey());
									//synchronized(lock1){
    ...
    protected String handleSep(){
    	return handleSep("0x1F");
    }
    ...
    //有些同步是不需要有删除动作，如“漫画、动画、主题、资讯”这些只增加不删除，不会有坏影响。而商品表就需要删除不是今天（依据flow_time）的数据。
	...
	protected abstract void insert(BufferQueue queue,VO vo, StatisticsCallback statisticsCallback);

 ...
 * 
 */
public class RBookLoadTimer {
	/**
	 * 记录日志的实例对象
	 */
	private static JLogger LOG = LoggerFactory.getLogger(RBookLoadTimer.class);
	private static RBookLoadTimer timer = new RBookLoadTimer();

	private RBookLoadTimer() {

	}

	public static RBookLoadTimer getInstance() {
		return timer;
	}
	...
	public void start() {
		...
		// 从配置项中获取配置值
		try {
			String rbStartTime = BaseBookConfig.get("BaseBookSynStartTime");
		...
		timer.schedule(new RBookLoadTask(), firstStartTime, taskIntervalMS);
		if (LOG.isDebugEnabled()) {
			LOG.debug("schedule a BaseBookLoadTask,first start time is:"
					+ firstStartTime);
		}
	}
